...
class Aliens2:
    def __init__(self, settings, screen, alien_group2, ship_height,stats, game):
        self.settings = settings
        self.aliens = alien_group2
        self.stats = stats
        self.screen = screen
        self.game = game
    ...
    def create_fleet(self):
        ...
        for y in range(4):
            ...
            for x in range(5):
                alien = Alien(settings=settings, screen=screen, number=0+a, x=0+n-b, y=580+d)
...
class Alien(Sprite):   # INHERITS from SPRITE
    images = [[pg.image.load('anim/p' + str(number) + str(i) + '.png') for i in range(2)] for number in range(5)]
    ...
    def check_edges(self):
        ...
        return r.right >= rscreen.right+300
    ...
    def draw(self):
        ...
        image = pg.transform.rotozoom(image, 0, 0.8 )
        rect = image.get_rect()
